...
coded table and assert it matches pilot/P13_results.json. Also verifies internal consistency.
Exit 0 on success. Run twice + diff P13_results.json for the byte-identical reproducibility gate.
"""
...
def main():
    ...
    R=json.load(open(C.RESULTS_JSON))
    fails=[]
    ...
    print(f"CHECK_CLAIMS: PASS — {N} incidents; T1-T4=({dist['T1']},{dist['T2']},{dist['T3']},{dist['T4']}); "
          f"all headline rates, CIs, strata sums, and accounting counts verified against P13_results.json.")
