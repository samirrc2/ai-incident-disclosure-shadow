#!/usr/bin/env python3
"""p13_20_analysis — compute all manuscript numbers from the coded table. Deterministic; no network.
Input : data/P13_disclosure_coding.csv (307 primary domestic-listed incidents, T1-T4 coded)
Output: pilot/P13_results.json  (machine-readable, consumed by check_claims.py and the manuscript)
        pilot/P13_results.md    (human-readable results)
"""
...
def main():
    ...
    R["robustness_note"]=("Window-length sensitivity (6/12/24 mo) and coder-1-vs-resolved are computed "
        "on the reliability subsample in p13_21 (kappa) once pass-2 raw-filing coding completes; "
        "primary coding uses the pre-registered 12-month window.")

    # cyber benchmark (Debevoise, disclosure-under-mandate; see recon P13_RECON.md section 6)
    ...
    (C.RESULTS_JSON).write_text(json.dumps(R,indent=2))
    # markdown
    h=R["headline"]
    def pct(b): return f"{b['rate']*100:.1f}% [{b['ci95'][0]*100:.1f}, {b['ci95'][1]*100:.1f}]"
    L=[f"# P13 Results (N={N} US-listed-matched AI incidents, 2019–2026)","",
       "## Four-tier disclosure distribution",
       "| Tier | meaning | n | share |","|---|---|---|---|",
       f"| T1 | disclosed-specific | {R['distribution']['T1']} | {pct(h['T1_specific'])} |",
       f"| T2 | legal-proceedings-only | {R['distribution']['T2']} | {rate_block(rows,T2)['rate']*100:.1f}% |",
    ...
    for g,b in R["by_severity"].items():
        L.append(f"| {g} | {b['n']} | {b['T1']['rate']*100:.1f}% | {b['T1T2']['rate']*100:.1f}% | {b['shadow_T3T4']['rate']*100:.1f}% |")
