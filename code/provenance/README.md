...
- `p13_03_edgar_search.py` — emits the exact, logged `efts.sec.gov` query URLs per incident
  (specific + legal-proceedings + generic term families). With `--run` (network) it re-executes them.
- `p13_lib.py` — the deterministic EDGAR-URL builder + disclosure-window helper.
