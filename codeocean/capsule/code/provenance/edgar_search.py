#!/usr/bin/env python3
"""edgar_search — build the deterministic EDGAR full-text search plan for the pilot.

Reads a pilot subsample CSV with columns:
  incident_id, CIK, incident_date, specific_terms (| -separated phrases)
For each incident, emits (to pilot/edgar_query_manifest.csv):
...
from shadow_lib import efts_url, disclosure_window, GENERIC_AI_TERMS
...
def main():
    ...
    sub = Path(args[0]) if args else (PILOT / "pilot_subsample.csv")
    if not sub.exists():
        sys.exit(f"subsample not found: {sub}")
    plan = build(sub)
    man = PILOT / "edgar_query_manifest.csv"
