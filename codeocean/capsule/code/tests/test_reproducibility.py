...
def test_extended_reliability():
    RL = _load(RES / "reliability_ext.json")
...
def test_resolution_regenerated():
    assert (RES / "incident_firm_map.regenerated.csv").exists()
    prim = [r for r in csv.DictReader(open(RES / "incident_firm_map.regenerated.csv"))
